...
Übersichtlichkeit halber ausgelassen.
...
Wenn die übergebene Liste sowohl Elemente vom Typ \lstinline!int! als auchvom
Typ \lstinline!float! enthält,
konvertiert NumPy in eine Fließkommazahl. Wie viele Bit für einen Integer
beziehungsweise ein Float zur Verfügung stehen, ist von der Prozessorarchitektur
abhängig. Diese beträgt bei aktuellen Architekturen in der Regel $64$ Bit.

\subsection{Konstanten und Funktionen}
\label{numpy:subsection:constantsAndFunctions}
Für die mathematische Anwendungen stehenh Konstanten zur Verfügung,
...
angewendet, so dass wir keine Schleifen implementieren müssen:
\begin{lstlisting}
...
\end{lstlisting}
Diese Implementierung ist nicht nur für einen übersichtlichen Code nützlich, 
sie ist auch extrem performant.

Es gibt Umrechnungsfunktionen, zum Beispiel von Bogenmaß in Gradmaß:
\begin{lstlisting}
>>> a = np.array([0, np.pi/6, np.pi/4, np.pi/3, np.pi/2])
>>> np.degrees(a)
[ 0., 30., 45., 60., 90.]
...
\lstinline!np.prod()! für Produkte:
\begin{lstlisting}
>>> a = np.array([1, 2, 3])
>>> np.sum(a)
6
...
\lstinline!range! bekannt erzeugen. Die Funktion hierzu heißt \lstinline!np.arange()!.
Der Datentyp der erzeugten Folge wird durch die Datentypen der übergebenen
Werte festgelegt:
\begin{lstlisting}
>>> np.arange(1,6)
[1, 2, 3, 4, 5]
...
angegebenen Wert, ausschließlich des letzten Werts. Als Schrittweite wird
...
NumPy hat weitere nützliche Funktionen für die Erzeugung von Feldern.
Sehr häufig benötigt man äquidistante Werte in einem Intervall.
...
dies an die Funktion übergeben:
\begin{lstlisting}
...
Der Parameter bestimmt die Länge des Arrays, der Datentyp ist standardmäßig
...
Quadruple Precision und 128 Bit Länge. Für die Mantisse stehen beim
...
\lstinline!TemporaryFile! verhält sich wie eine normale Datei, mit dem
...
Neben den Datenstruktoren enthält NumPy eine ganze Reihe von Funktionen
für die lineare Algebra. Das Skalarprodukt und allgemein die Matrixmultiplikation
