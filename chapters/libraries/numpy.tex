\section{NumPy}
\label{numpy}
\nump{} ist eine Python-Bibliothek für wissenschaftliches Rechnen.
...
Um \nump{} zu installieren, kann der Befehl \lstinline!pip install numpy!
verwendet werden.

\subsection{Arrays}
\label{numpy:subsection:arrays}

Der Array-Datentyp von \nump{} heißt \lstinline!numpy.ndarray!.
...
Der \nump{}-eigene Datentyp ermöglicht es, Arrays direkt über den
...
a + 3 # [4 5 6]
\end{lstlisting}
\nump{} wird hierbei mit dem Namen \lstinline!np! importiert. Damit folgt dieses
Tutorial der Konvention aus der Dokumentation von \nump{}.
...
Um ein \nump{}-Array zu erzeugen, wird ein \lstinline!list!-Objekt an die Funktion
\lstinline!np.array()! übergeben. Dabei werden alle Elemente im
\lstinline!list!-Objekt in einem Datentyp von \nump{} konvertiert. Um den Datentyp
...
konvertiert \nump{} in einen Fließkommatyp. Wie viele Bit für einen Integer
...
\nump{} unterstützt eine Vielzahl an mathematischen Funktionen, darunter unter
anderem Folgende:
...
mittels \lstinline!np.array! in ein \nump{}-Array konvertiert.
...
konvertieren, bietet \nump{} die Funktion \lstinline!np.fromiter()! an. Der zweite
...
\pagebreak
\uebungTutorial{numpy_01}{numpy_02}
